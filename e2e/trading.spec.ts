...
test('selects a ticker, trades through repricing, and realizes P&L', async ({ page }) => {
  ...
  const microsoftRow = page.getByRole('row', { name: /MSFT Microsoft Corporation/ });
  ...
  await expect(holdingsTable.getByRole('row', { name: /MSFT Microsoft Corporation/ })).toContainText('$349.96');

  await page.getByRole('button', { name: '+1 Day' }).click();
  await expect(page.getByLabel('Simulation date')).toHaveValue('2024-01-03');
  await expect(holdingsTable.getByRole('row', { name: /MSFT Microsoft Corporation/ })).toContainText('$350.81');
  await expect(holdingsTable.getByRole('row', { name: /MSFT Microsoft Corporation/ })).toContainText('+$8.50');
